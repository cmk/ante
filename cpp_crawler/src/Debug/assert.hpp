...
#ifdef FINAL
#	define DE_ASSERT(condition, ...) static_cast<void(0)>
#else
...
#ifdef FINAL
#	define DE_VERIFY(condition, ...) if (condition) static_cast<void>(0) else static_cast<void>(0)
#else
#	define DE_VERIFY(condition, ...) DE_ASSERT(condition, __VA_ARGS__)
#endif

#ifdef FINAL
#	define DE_THROW(exception) throw exception
#else
#	define DE_THROW(exception) do { if (::assert::ScopedSetBreakOnThrow::break_on_throw()) DEBUG_BREAK_IF_DEBUGGER_ATTACHED(); throw exception; } while (false)
#endif
